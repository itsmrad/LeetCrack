...
import { useLeetCodeStore } from "@/store/store";
import { prepareAIInput } from "@/utils/aiPayload";
import { sendToAIAPI } from "@services/aiService";

export async function handleLeetCodeContentScript() {
	...
	try {
		...
		if (leetcodeData.success) {
			...
			console.error("❌ Failed to process LeetCode data:", leetcodeData.error);
		}

		useLeetCodeStore.getState().setLoading(true);

		if (leetcodeData.success) {
			useLeetCodeStore.getState().setData(leetcodeData);
			useLeetCodeStore.getState().setError(null);
		} else {
			useLeetCodeStore
				.getState()
				.setError(leetcodeData.error ?? "Unknown error");
		}
		useLeetCodeStore.getState().setLoading(false);

		const aiInput = prepareAIInput(leetcodeData);

		if (aiInput) {
			const aiResponse = await sendToAIAPI(aiInput);
